...
local function get_params(line1, line2)
	...
	local sapling_remotebookmark_template =
		'word(0,sub("\\w+/", "", remotebookmarks))'
	-- use the remote branch name if it exists, otherwise use the commit hash
	local sapling_ref_template = '{ifeq(remotebookmarks, "", "{node}",' ..
		                             sapling_remotebookmark_template .. ' )}'
	-- TODO: check if commit is pushed, and if not fall back to `main` branch
	local ref = sl('log -r . -T \'' .. sapling_ref_template .. "'",
...
local function get_repo_url_from_sl_path(path)
	local function trim_to(str, pattern)
		...
		if start then return string.sub(str, start + string.len(pattern)) end
		return str
	end
	...
	if repo_url_end then repo_url = string.sub(repo_url, 0, repo_url_end - 1) end
	return repo_url
end
...
function Open_File_Cmd(params)
	local open_cmd = "open"
	local os = params.os
	if os == "Linux" then open_cmd = "xdg-open" end
...
function Copy_File_Cmd(params)
	...
	local os = params.os
	if os == "Linux" then copy_cmd = "xclip -selection clipboard" end
...
function Get_remote_file_url(params)
	...
	local relative_filepath = string.sub(absolute_filepath,
	                                     string.len(repo_root) + 2)

	local line_number_fragment = "#L" .. line_number_start
	if line_number_end and line_number_end ~= line_number_start then
		line_number_fragment = line_number_fragment .. "-L" .. line_number_end
	end

	local url = vim.fn.fnameescape("https://" ..
		                               get_repo_url_from_sl_path(remote_path) ..
		                               "/blob/" .. ref .. "/" .. relative_filepath ..
...
vim.api.nvim_create_user_command("OpenFile", Open_file,
                                 {range = true, nargs = 0})
...
vim.api.nvim_create_user_command("CopyFile", Copy_file,
                                 {range = true, nargs = 0})

--- TESTS
local function test_get_repo_url_from_remote_path()
	Assert_equals(get_repo_url_from_sl_path(
		              "ssh://git@gecgithub01.walmart.com/ce-orchestration/ce-smartlists.git"),
	              "gecgithub01.walmart.com/ce-orchestration/ce-smartlists")
	Assert_equals(get_repo_url_from_sl_path(
...
local function test_open_file_helper()
	local cmd = Open_File_Cmd({
		absolute_filepath = "~/dotfiles/.config/nvim/lua/open-remote.lua",
		repo_root = "~/dotfiles",
		line_number_start = 69,
		os = "Darwin",
		remote_path = "git@github.com/vegerot/dotfiles",
		ref = "main"
	})
	Assert_equals(cmd,
	              "open 'https://github.com/vegerot/dotfiles/blob/main/.config/nvim/lua/open-remote.lua\\#L69'")

	local cmd2 = Open_File_Cmd({
		absolute_filepath = "~/workspace/gitlab.com/gitlab-org/gitlab/.gitlab/CODEOWNERS",
		line_number_start = 42,
		repo_root = "~/workspace/gitlab.com/gitlab-org/gitlab",
		os = "Linux",
		remote_path = "https://gitlab.com/gitlab-org/gitlab.git",
		ref = "79da4d8f"
	})
	Assert_equals(cmd2,
	              "xdg-open 'https://gitlab.com/gitlab-org/gitlab/blob/79da4d8f/.gitlab/CODEOWNERS\\#L42'")

	-- visual line range
	local cmd3 = Open_File_Cmd({
		absolute_filepath = "~/dotfiles/.config/nvim/lua/open-remote.lua",
		repo_root = "~/dotfiles",
		line_number_start = 42,
		line_number_end = 69,
	...
	Assert_equals(cmd3,
	              "open 'https://github.com/vegerot/dotfiles/blob/main/.config/nvim/lua/open-remote.lua\\#L42-L69'")
end

local function test_copy_file_helper()
	local cmd = Copy_File_Cmd({
		absolute_filepath = "~/dotfiles/.config/nvim/lua/open-remote.lua",
		repo_root = "~/dotfiles",
		line_number_start = 69,
		os = "Linux",
	...
	Assert_equals(cmd,
	              "printf 'https://github.com/vegerot/dotfiles/blob/main/.config/nvim/lua/open-remote.lua\\#L69' | xclip -selection clipboard")

	local cmd2 = Copy_File_Cmd({
		absolute_filepath = "~/dotfiles/.config/nvim/lua/open-remote.lua",
		repo_root = "~/dotfiles",
		line_number_start = 69,
		os = "Darwin",
	...
	Assert_equals(cmd2,
	              "printf 'https://github.com/vegerot/dotfiles/blob/main/.config/nvim/lua/open-remote.lua\\#L69' | pbcopy")
...
local function e2e_test()
	...
	Assert_equals(cmd,
	              "xdg-open 'https://github.com/testuser/testrepo/blob/main/foo.txt\\#L2'")
...
local function e2e_test_2()
	local cmd = Get_open_file_cmd()
	local want =
		"xdg--open 'https://github.com/testuser/testrepo/blob/(%x+)/foo.txt\\#L2'"
...
local function e2e_test_visual_select_range(lines)
	local cmd = Get_open_file_cmd(lines.line1, lines.line2)
	Assert_equals(cmd,
	              "xdg-open 'https://github.com/testuser/testrepo/blob/main/foo.txt\\#L2-L4'")
	-- for some reason neovim (libuv in particular) crashes when using `vim.cmd("quit")`
	os.exit()

end
vim.api.nvim_create_user_command("OpenRemoteTestE2eVisualSelectRange",
                                 e2e_test_visual_select_range,
                                 {nargs = 0, range = true})
